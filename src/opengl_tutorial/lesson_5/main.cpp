...
int main()
{
    ...
    glm::mat4 viewMat4 = glm::lookAt(
        glm::vec3(4, 4, 4), // Координаты камеры в мировом пространстве
        glm::vec3(0, 1, 0), // и куда смотрит
    ...
    do {
        ...
        // Привзяка текстуры к Texture Unit 0
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture);
        // Установка myTextureSampler для использования в Texture Unit 0
        ...
        glBindBuffer(GL_ARRAY_BUFFER, uvBuffer);
        // Второй буффер аттрибутов - UV координаты
        glVertexAttribPointer(
            1, // значение для layout в шейдере
            2, // размер аттрибута. В данном случае двухмерная UV координата
            GL_FLOAT, // тип координат вершин в частности и тип данных аттрибута в общем
            GL_FALSE, // нужно ли нормализовать?
            0, // шаг между атрибутами вершин 
            nullptr // смещение того места где начинаются данные в буффере
    ...
    // Закрыть OpenGL окно и завершить GLFW
    glfwTerminate();
}